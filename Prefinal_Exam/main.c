	...
	for(i = 0; i < MAX_RACK; ++i) {
		...
		if(vm.rackItems[i].itemCount == 0) {
			...
			for(j = 0; j < vm.rackItems[i].itemCount; j++) {
				printf("\tItem - %1d %12s - Php %12.2lf - %.2d/%.2d/%d\n", j, 
				vm.rackItems[i].list[j].prodName, 
				vm.rackItems[i].list[j].prodPrice, 
				vm.rackItems[i].list[j].expiryDate.day,
				vm.rackItems[i].list[j].expiryDate.month, 
				vm.rackItems[i].list[j].expiryDate.year);
			}
		}	
	} 
...
Boolean addProduct(VendingMachine *vm, int rackNumber, Product prodItem) {
	...
	if (vm->shelves[rackNumber] == FALSE) {
		for (j = 0; j < vm->rackItems[rackNumber].itemCount; j++) {
			if (date_difference(vm->rackItems[rackNumber].list[j].expiryDate, prodItem.expiryDate) < 0) {
				temp = vm->rackItems[rackNumber].list[j];
				vm->rackItems[rackNumber].list[j] = prodItem;
				prodItem = temp;
			}
		}
...
Boolean purchaseProduct(VendingMachine *vm, int rackNumber, float amount) {
	...
	if(vm->rackItems[rackNumber].itemCount == 0) {
		...
		for(i = 0; i < vm->rackItems[rackNumber].itemCount; ++i) {
			if(vm->rackItems[rackNumber].list[i].prodPrice <= amount) {
				printf("Please enjoy your food.\n");
				printf("Your have a change of Php %.2lf\n", amount-=vm->rackItems[rackNumber].list[i].prodPrice);
				vm->totalMoney += vm->rackItems[rackNumber].list[i].prodPrice;
